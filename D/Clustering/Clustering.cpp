...
ImageCollection * Clustering::update(unsigned int C, AssignmentDefinition * assignments) {
    unsigned int D = (*collection)[0].size(); // 784
...
void AssignmentDefinition::get_og_centers(ImageCollection* collection) {

    for (unsigned i = 0; i < centers->size(); i++) {
        vector<int> & temp = (*centers)[i];
...
vector<float> Clustering::silhouette(AssignmentDefinition * def) {
    ...
    unsigned int C = def->centers->size();

    vector<float> cluster_silhouette;

    float totalS = 0.0;

    for (unsigned int i = 0; i < C; i++) {
        float s = 0;
        for (unsigned int j = 0; j<def->cluster_to_points[i].size(); j++) {
            s+=image_silhouette[j];
            
        }
        //cout<<"my s "<<s;
            
        if (def->cluster_to_points[i].size() > 0) {
            s /= def->cluster_to_points[i].size();
            
           // cout<<"   /   "<<def->cluster_to_points[i].size()<<endl;
            cluster_silhouette.push_back(s);
    ...
    float result = totalS / C;
    
    cluster_silhouette.push_back(result);
