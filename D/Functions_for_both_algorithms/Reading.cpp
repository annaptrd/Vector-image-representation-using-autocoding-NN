        ...
        while (1) {
            ...
            for (int j = 0; j<*columns; j++) {
                for (int k = 0; k<*rows; k++) {
                    ...
                    file.read((char*) &pixel, sizeof (pixel)); //cout<<"PPP1:   "<<(int)pixel<<endl;
                    if (file.eof()) {
                        flag = 1;
                        break;
                    }
        ...
        *columns = ReverseInt(*columns);
        ...
        while (1) {
            vector<int> image;
                for (int j = 0; j<*columns; j++) {
                for (int k = 0; k<*rows; k++) {
                    unsigned char pixel1 = 0,pixel2=0, pixel;
              //    unsigned short pixel;
                    file.read((char*) &pixel1, sizeof (pixel1)); //cout<<"P1:   "<<int(pixel1)<<endl;
                    file.read((char*) &pixel2, sizeof (pixel2)); //cout<<"P1:   "<<int(pixel2)<<endl;
